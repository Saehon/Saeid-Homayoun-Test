...
def count(d, toks):
    return sum(t in d for t in toks)
...
def extract(text):
    ...
    return {'word_count':len(toks),'sentence_count':len(sentences(text)),'avg_sentence_length':len(toks)/max(1,len(sentences(text))),'flesch_reading_ease':flesch(text),'sha256':hashlib.sha256(text.encode()).hexdigest(),'uncertainty_per_100_words':100*count(UNCERTAINTY,toks)/n,'modal_per_100_words':100*count(MODAL,toks)/n,'innovation_per_100_words':100*count(INNOVATION,toks)/n,'negative_per_100_words':100*count(NEGATIVE,toks)/n,'positive_per_100_words':100*count(POSITIVE,toks)/n,'litigious_per_100_words':100*count(LITIGIOUS,toks)/n,'human_capital_per_100_words':100*count(HUMAN,toks)/n}
